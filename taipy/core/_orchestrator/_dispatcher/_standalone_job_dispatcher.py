...
class _StandaloneJobDispatcher(_JobDispatcher):
    ...
    def _can_execute(self) -> bool:
        ...
        with self._nb_available_workers_lock:
            self._logger.debug(f"can execute a job ? {self._nb_available_workers}")
    ...
    def _dispatch(self, job: Job):
        ...
        with self._nb_available_workers_lock:
            ...
            self._logger.debug(f"Changing nb_available_workers to {self._nb_available_workers} from dispatch")
    ...
    def _update_job_status_from_future(self, job: Job, ft):
        with self._nb_available_workers_lock:
            self._nb_available_workers += 1
            self._logger.debug(f"Changing nb_available_workers to {self._nb_available_workers} from callback")
