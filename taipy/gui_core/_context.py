    ...
    def process_event(self, event: Event):
        if event.entity_type == EventEntityType.SCENARIO:
            ...
            try:
                ...
                if sequence and hasattr(sequence, "parent_ids") and sequence.parent_ids:  # type: ignore[attr-defined]
                    self.gui._broadcast(
                        _GuiCoreContext._CORE_CHANGED_NAME,
                        {"scenario": [x for x in sequence.parent_ids]},  # type: ignore[attr-defined]
                    )
