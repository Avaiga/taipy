...
class Executor:
    def __init__(self, parallel_execution: bool, max_number_of_parallel_execution: Optional[int]):
        if parallel_execution:
            self.__executor = ProcessPoolExecutor(max_number_of_parallel_execution)
            self.__nb_worker_available = self.__executor._max_workers
        else:
            self.__executor = Synchronous()
