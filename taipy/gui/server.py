...
class Server(Flask):
    ...
    def render(self, html_fragment, style, dark_mode):
        ...
        data = {"jsx": template_str, "style": (
            (style + os.linesep) if style else ''), "darkMode": dark_mode}
    ...
    def render_react_route(self, routes):
        ...
        for route in routes:
            router += "<Route path=\"/" + route + \
                "\" exact component={TaipyRendered} />"
        router += "<Route path=\"/404\" exact component={NotFound404} /><Redirect to=\"/" + routes[0] + "\" /></Switch></Router>"
