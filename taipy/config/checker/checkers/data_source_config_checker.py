    ...
    def _check_storage_type(self, data_source_config_name: str, data_source_config: DataSourceConfig):
        if data_source_config.storage_type not in ["csv", "pickle", "in_memory", "sql"]:
            self._error(
                ...
                f"storage_type field of DataSource {data_source_config_name} must be either csv, sql, pickle or in_memory.",
            )
    ...
    def _check_scope(self, data_source_config_name: str, data_source_config: DataSourceConfig):
        if not isinstance(data_source_config.scope, Scope):
            self._error(
                ...
                f"scope field of DataSource {data_source_config_name} must be populated with a Scope value.",
            )
