...
class _Config:
    ...
    def __update_entity_configs(self, sub_configs, other_sub_configs, _class):
        ...
        for name, ds_config in other_sub_configs.items():
            if name != self.DEFAULT_KEY:
                if name in sub_configs:
                    sub_configs[name].update(ds_config.to_dict(), sub_configs.get(self.DEFAULT_KEY))
                else:
                    sub_configs[name] = copy(ds_config)
                    sub_configs[name].update(ds_config.to_dict(), sub_configs.get(self.DEFAULT_KEY))
