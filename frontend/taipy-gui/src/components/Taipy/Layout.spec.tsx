...
describe("Layout Component", () => {
    ...
    it("displays the default value", async () => {
        const { getByText } = render(
            <Layout gap="1rem" columns="1 1 1 1">
                <div>foo</div>
            </Layout>
        );
    ...
    it("keeps the grid definition", async () => {
        const { getByText } = render(
            <Layout gap="1rem" columns="10px 1em 1rem 1fr">
                <div>foo</div>
            </Layout>
        );
        const elt = getByText("foo");
    ...
    it("handles the concise column expression", async () => {
        const { getByText } = render(
            <Layout gap="1rem" columns="1*3px 1 3*1em 2 3 3*2">
                <div>foo</div>
            </Layout>
        );
        const elt = getByText("foo");
    ...
    it("handles fit-content columns", async () => {
        const { getByText } = render(
            <Layout gap="1rem" columns="fc fc">
                <div>foo</div>
            </Layout>
        );
        const elt = getByText("foo");
        expect(elt.parentElement).toHaveStyle({ "grid-template-columns": "fit-content fit-content" });
    });
});
