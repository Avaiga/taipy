...
describe("PaginatedTable Component", () => {
    ...
    it("displays the right info for class", async () => {
        const { getByText } = render(<PaginatedTable value={undefined} columns={tableColumns} className="taipy-table" />);
    ...
    it("dispatch a well formed message on page change", async () => {
        ...
        const { getByLabelText, rerender } = render(<TaipyContext.Provider value={{ state, dispatch }}>
