...
interface ChatProps extends TaipyActiveProps {
    ...
    defaultKey?: string;
    pageSize?: number;
    onImageUpload?: (file: File) => void; // Added onImageUpload prop
}
...
const Chat = (props: ChatProps) => {
    const { id, updateVarName, senderId = "taipy", onAction, withInput = true, defaultKey = "", pageSize = 50, onImageUpload } = props;
        ...
        <Tooltip title={hover || "" || `rowCount: ${rowCount}`}>
            <Paper className={className} sx={boxSx} id={id}>
                <Grid container rowSpacing={2} sx={gridSx} ref={scrollDivRef}>
                    {/* Existing grid items and mapping... */}
                </Grid>
                ...
                {withInput ? (
                    <Box>
                        <TextField
                            ...
                            sx={inputSx}
                        />
                        <input type="file" accept="image/*" onChange={handleImageUpload} /> {/* Image upload button */}
                    </Box>
                ) : null}
            </Paper>
        </Tooltip>
