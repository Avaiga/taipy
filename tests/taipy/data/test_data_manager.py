...
class TestDataManager:
    ...
    def test_create_and_get_csv_data_node(self):
        ...
        assert isinstance(DataManager.get(csv_dn.id), CSVDataNode)
        ...
        assert DataManager.get(csv_dn.id).scope == Scope.PIPELINE
        assert DataManager.get(csv_dn.id).scope == csv_dn.scope
        assert DataManager.get(csv_dn.id).parent_id is None
        assert DataManager.get(csv_dn.id).parent_id == csv_dn.parent_id
        assert DataManager.get(csv_dn.id).last_edition_date is None
        assert DataManager.get(csv_dn.id).last_edition_date == csv_dn.last_edition_date
        assert DataManager.get(csv_dn.id).job_ids == []
        assert DataManager.get(csv_dn.id).job_ids == csv_dn.job_ids
        assert not DataManager.get(csv_dn.id).is_ready_for_reading
        assert DataManager.get(csv_dn.id).is_ready_for_reading == csv_dn.is_ready_for_reading
        assert len(DataManager.get(csv_dn.id).properties) == 3
        assert not DataManager.get(csv_dn.id).properties["cacheable"]
        assert DataManager.get(csv_dn.id).properties.get("path") == "bar"
        assert DataManager.get(csv_dn.id).properties.get("has_header")
        assert DataManager.get(csv_dn.id).properties == csv_dn.properties
        ...
        assert DataManager.get(csv_dn).scope == Scope.PIPELINE
        assert DataManager.get(csv_dn).scope == csv_dn.scope
        assert DataManager.get(csv_dn).parent_id is None
        assert DataManager.get(csv_dn).parent_id == csv_dn.parent_id
        assert DataManager.get(csv_dn).last_edition_date is None
        assert DataManager.get(csv_dn).last_edition_date == csv_dn.last_edition_date
        assert DataManager.get(csv_dn).job_ids == []
        assert DataManager.get(csv_dn).job_ids == csv_dn.job_ids
        assert not DataManager.get(csv_dn).is_ready_for_reading
        assert DataManager.get(csv_dn).is_ready_for_reading == csv_dn.is_ready_for_reading
        assert len(DataManager.get(csv_dn).properties) == 3
        assert not DataManager.get(csv_dn.id).properties["cacheable"]
        assert DataManager.get(csv_dn).properties.get("path") == "bar"
        assert DataManager.get(csv_dn).properties.get("has_header")
        assert DataManager.get(csv_dn).properties == csv_dn.properties
    ...
    def test_create_and_get_in_memory_data_node(self):
        ...
        assert isinstance(DataManager.get(in_mem_dn.id), InMemoryDataNode)
        ...
        assert DataManager.get(in_mem_dn.id).parent_id == in_mem_dn.parent_id
        assert DataManager.get(in_mem_dn.id).last_edition_date is not None
        assert DataManager.get(in_mem_dn.id).last_edition_date == in_mem_dn.last_edition_date
        assert DataManager.get(in_mem_dn.id).job_ids == []
        assert DataManager.get(in_mem_dn.id).job_ids == in_mem_dn.job_ids
        assert DataManager.get(in_mem_dn.id).is_ready_for_reading
        assert DataManager.get(in_mem_dn.id).is_ready_for_reading == in_mem_dn.is_ready_for_reading
        assert len(DataManager.get(in_mem_dn.id).properties) == 2
        assert DataManager.get(in_mem_dn.id).properties["cacheable"]
        assert DataManager.get(in_mem_dn.id).properties.get("other_data") == "foo"
        assert DataManager.get(in_mem_dn.id).properties == in_mem_dn.properties
        ...
        assert DataManager.get(in_mem_dn).parent_id == in_mem_dn.parent_id
        assert DataManager.get(in_mem_dn).last_edition_date is not None
        assert DataManager.get(in_mem_dn).last_edition_date == in_mem_dn.last_edition_date
        assert DataManager.get(in_mem_dn).job_ids == []
        assert DataManager.get(in_mem_dn).job_ids == in_mem_dn.job_ids
        assert DataManager.get(in_mem_dn).is_ready_for_reading
        assert DataManager.get(in_mem_dn).is_ready_for_reading == in_mem_dn.is_ready_for_reading
        assert len(DataManager.get(in_mem_dn).properties) == 2
        assert DataManager.get(in_mem_dn.id).properties["cacheable"]
        assert DataManager.get(in_mem_dn).properties.get("other_data") == "foo"
        assert DataManager.get(in_mem_dn).properties == in_mem_dn.properties
    ...
    def test_create_and_get_pickle_data_node(self):
        ...
        # - No parent id
        # - no default data
        dn_config = Config.add_data_node(
            name="plop", storage_type="pickle", scope=Scope.BUSINESS_CYCLE, cacheable=False
        )
        pickle_dn = DataManager._create_and_set(dn_config, None)

        assert isinstance(pickle_dn, PickleDataNode)
        assert isinstance(DataManager.get(pickle_dn.id), PickleDataNode)
        ...
        assert DataManager.get(pickle_dn.id).scope == Scope.BUSINESS_CYCLE
        assert DataManager.get(pickle_dn.id).scope == pickle_dn.scope
        assert DataManager.get(pickle_dn.id).parent_id is None
        assert DataManager.get(pickle_dn.id).parent_id == pickle_dn.parent_id
        assert DataManager.get(pickle_dn.id).last_edition_date is None
        assert DataManager.get(pickle_dn.id).last_edition_date == pickle_dn.last_edition_date
        assert DataManager.get(pickle_dn.id).job_ids == []
        assert DataManager.get(pickle_dn.id).job_ids == pickle_dn.job_ids
        assert not DataManager.get(pickle_dn.id).is_ready_for_reading
        assert DataManager.get(pickle_dn.id).is_ready_for_reading == pickle_dn.is_ready_for_reading
        assert len(DataManager.get(pickle_dn.id).properties) == 1
        assert not DataManager.get(pickle_dn.id).properties["cacheable"]
        assert DataManager.get(pickle_dn.id).properties == pickle_dn.properties
        ...
        assert DataManager.get(pickle_dn).scope == Scope.BUSINESS_CYCLE
        assert DataManager.get(pickle_dn).scope == pickle_dn.scope
        assert DataManager.get(pickle_dn).parent_id is None
        assert DataManager.get(pickle_dn).parent_id == pickle_dn.parent_id
        assert DataManager.get(pickle_dn).last_edition_date is None
        assert DataManager.get(pickle_dn).last_edition_date == pickle_dn.last_edition_date
        assert DataManager.get(pickle_dn).job_ids == []
        assert DataManager.get(pickle_dn).job_ids == pickle_dn.job_ids
        assert not DataManager.get(pickle_dn).is_ready_for_reading
        assert DataManager.get(pickle_dn).is_ready_for_reading == pickle_dn.is_ready_for_reading
        assert len(DataManager.get(pickle_dn).properties) == 1
        assert not DataManager.get(pickle_dn.id).properties["cacheable"]
    ...
    def test_delete(self):
        ...
        DataManager.set(dn_3)
        assert len(DataManager.get_all()) == 3
        DataManager.delete(dn_1.id)
        assert len(DataManager.get_all()) == 2
        assert DataManager.get(dn_2.id).id == dn_2.id
        assert DataManager.get(dn_3.id).id == dn_3.id
        with pytest.raises(NonExistingDataNode):
            DataManager.get(dn_1.id)
