...
def test_slider_md(gui: Gui, helpers):
    ...
    md_string = "<|{x}|slider|>"
    expected_list = ["<Slider", 'updateVarName="x', "defaultValue={10}", "value={x}"]
...
def test_slider_text_anchor_md(gui: Gui, helpers):
    ...
    expected_list = ["<Slider", 'updateVarName="x"', "value={x}", 'textAnchor="none"']
    helpers.test_control_md(gui, md_string, expected_list)
...
def test_slider_text_anchor_default_md(gui: Gui, helpers):
    ...
    expected_list = ["<Slider", 'updateVarName="x"', "value={x}", 'textAnchor="bottom"']
    helpers.test_control_md(gui, md_string, expected_list)
