...
def test_a_button_pressed(gui: Gui, helpers):
    ...
    assert gui._get_user_data()._get_all_scopes()[sid].x == 10  # type: ignore
    assert gui._get_user_data()._get_all_scopes()[sid].text == "hi"  # type: ignore
    ws_client.emit("message", {"client_id": sid, "type": "A", "name": "my_button", "payload": "do_something"})
    assert gui._get_user_data()._get_all_scopes()[sid].text == "a random text"
    assert gui._get_user_data()._get_all_scopes()[sid].x == 20  # type: ignore
