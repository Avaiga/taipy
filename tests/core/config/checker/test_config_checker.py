...
def test__error():
    ...
    assert collector.errors[0] == Issue(IssueCollector._ERROR_LEVEL, "field", 17, "my message", "ConfigChecker")
...
def test__warning():
    ...
    assert len(collector.infos) == 0
    assert collector.warnings[0] == Issue(IssueCollector._WARNING_LEVEL, "field", 17, "my message", "ConfigChecker")
...
def test__info():
    ...
    assert len(collector.errors) == 0
    assert len(collector.warnings) == 0
    assert collector.infos[0] == Issue(IssueCollector._INFO_LEVEL, "field", 17, "my message", "ConfigChecker")
