...
def test_get_sorted_tasks():
    ...
    data_node_6 = DataNode("quuz", Scope.PIPELINE, "s6")
    data_node_7 = DataNode("corge", Scope.PIPELINE, "s7")
    data_node_8 = DataNode("hugh", Scope.PIPELINE, "s7")
    ...
    pipeline = Pipeline("plugh", {}, [task_4, task_2, task_1, task_3, task_5], PipelineId("p1"))
    # s1 ---
    #       |
    #       |---> t1 ---|      -----> t3
    #       |           |      |
    ...
    # t2 ---> s5
    # s8 ---> t5
    assert pipeline._get_sorted_tasks() == [[task_2, task_1], [task_4, task_3], [task_5]]
