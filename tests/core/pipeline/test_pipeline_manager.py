...
def test_do_not_recreate_existing_pipeline_except_same_config():
    ...
    dn_output_config_scope_scenario_4 = Config.configure_data_node("my_output_4", "in_memory", scope=Scope.SCENARIO)
    task_config_4 = Config.configure_task(
        "task_config_4", print, dn_input_config_scope_pipeline_4, dn_output_config_scope_scenario_4
    )
    ...
    pipeline_14 = _PipelineManager._get_or_create(pipeline_config_4, None, "random_scenario")
    assert len(_PipelineManager._get_all()) == 7
    assert pipeline_14.id != pipeline_13.id
    assert pipeline_14.id != pipeline_12.id
    pipeline_15 = _PipelineManager._get_or_create(pipeline_config_4, "a_cycle", "random_scenario")
    assert len(_PipelineManager._get_all()) == 7
    ...
    dn_output_config_scope_pipeline_5 = Config.configure_data_node("my_output_5", "in_memory", scope=Scope.CYCLE)
    task_config_5 = Config.configure_task(
        "task_config_5", print, dn_input_config_scope_pipeline_5, dn_output_config_scope_pipeline_5
    )
    pipeline_config_5 = Config.configure_pipeline("pipeline_config_5", [task_config_5])

    pipeline_15 = _PipelineManager._get_or_create(pipeline_config_5)
    assert len(_PipelineManager._get_all()) == 8
    pipeline_16 = _PipelineManager._get_or_create(pipeline_config_5)
    assert len(_PipelineManager._get_all()) == 8
    assert pipeline_15.id == pipeline_16.id
    pipeline_17 = _PipelineManager._get_or_create(pipeline_config_5, "a_cycle", None)
    assert len(_PipelineManager._get_all()) == 9
    assert pipeline_15.id == pipeline_16.id
    assert pipeline_15.id != pipeline_17.id
    assert pipeline_16.id != pipeline_17.id
    pipeline_18 = _PipelineManager._get_or_create(pipeline_config_5, None, "a_scenario")
    assert len(_PipelineManager._get_all()) == 9
    assert pipeline_15.id == pipeline_16.id
    assert pipeline_15.id != pipeline_17.id
    assert pipeline_16.id != pipeline_17.id
    assert pipeline_18.id != pipeline_17.id
    assert pipeline_15.id == pipeline_18.id
    pipeline_19 = _PipelineManager._get_or_create(pipeline_config_5, "a_cycle", "a_scenario")
    assert len(_PipelineManager._get_all()) == 9
    assert pipeline_15.id == pipeline_16.id
    assert pipeline_15.id != pipeline_17.id
    assert pipeline_16.id != pipeline_17.id
    assert pipeline_18.id != pipeline_17.id
    assert pipeline_17.id == pipeline_19.id
    assert pipeline_18.id != pipeline_19.id
