...
class TestDataNodeConfigChecker:
    ...
    def test_check_required_properties(self):
        ...
        config._data_nodes["default"].storage_type = "sql"
        config._data_nodes["default"].properties = {key: f"the_{key}" for key in required_properties}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 0
        ...
        config._data_nodes["default"].storage_type = "json"
        config._data_nodes["default"].properties = {"default_path": "bar"}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 0
    ...
    def test_check_read_write_fct(self):
        ...
        config._data_nodes["default"].storage_type = "generic"
        config._data_nodes["default"].properties = {"write_fct": 12}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 2

        config._data_nodes["default"].storage_type = "generic"
        config._data_nodes["default"].properties = {"read_fct": 5}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 2

        config._data_nodes["default"].storage_type = "generic"
        config._data_nodes["default"].properties = {"write_fct": 9, "read_fct": 5}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 2

        config._data_nodes["default"].storage_type = "generic"
        config._data_nodes["default"].properties = {"write_fct": print, "read_fct": 5}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 1

        config._data_nodes["default"].storage_type = "generic"
        config._data_nodes["default"].properties = {"write_fct": 5, "read_fct": print}
        collector = IssueCollector()
        _DataNodeConfigChecker(config, collector)._check()
        assert len(collector.errors) == 1
